...
import json, modal
name = "realact23m_bs8192_lr4e-4"
extra = "--prefix-cache --grad-ckpt 0 --autocast-bf16 --log-steps 10 --grad-accum 16"
t = modal.Function.from_name("maemm-sft-8xb200", "train").spawn(run_name=name, data_dir="/data/banks/realact_short_20m_all", n_ckpts=10, epochs=1,
                                                                 batch_size=64, lr=4e-4, max_seq=160, backend="nccl", extra_args=extra)
e = modal.Function.from_name("maemm-eval-ckpt-mlp", "daemon").spawn(ckpt_dir=f"/data/sft_mix/{name}", tag=f"sft_{name}", rl_run_id="", wandb_name=f"{name}_eval",
                                                                    final_step=2808, extra_args="--eval-cache /data/eval_universal_ho/eval_sets_heldout_v2.pt")
json.dump({name: {"train": t.object_id, "eval": e.object_id, "eff_batch": 8192, "lr": 4e-4, "steps": 2808, "data": "/data/banks/realact_short_20m_all"}},
          open("/home/celeste/shared/overnight/sft_bs8192_ids.json", "w"), indent=1)
