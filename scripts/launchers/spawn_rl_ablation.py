...
INITS = {"init23m": "/data/sft_mix/realact20m_prefix_lr1e-4/final", "initbase": "none", "initmid": f"/data/sft_mix/{SFT_RUN}/final", "initboth": f"/data/sft_mix/{SFT_RUN_BOTH}/final"}
what = sys.argv[1]
if what == "sft":
    ...
    t = modal.Function.from_name("maemm-rl-disagg-base" if what in ("initbase", "initboth") else "maemm-rl-disagg-fast", "train").spawn(n_rollout=3, n_trainer=5, total_steps=300, extra_args=extra, pool_dir=POOL)
    ...
    d[what] = {"train": t.object_id, "eval": e.object_id, "run": run, "save": save, "init": INITS[what], "pool": POOL, "lr": "7e-6", "group_size": 8, "groups_per_step": 512, "steps": 300, "app": "maemm-rl-disagg-base" if what in ("initbase", "initboth") else "maemm-rl-disagg-fast"}
