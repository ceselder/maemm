...
Extra rl_disagg flags after `--`, e.g. `python3 spawn_rl_fullparam.py smoke 1e-6 -- --publish-mode fs`."""
import json
import os
import sys
import time
...
t = modal.Function.from_name(APP, "train").spawn(n_rollout=3, n_trainer=5, total_steps=steps, extra_args=extra, pool_dir=POOL,
...
rec = {"train": t.object_id, "run": run, "save": save, "policy_base": POLICY_BASE, "pool": POOL, "lr": lr, "group_size": 8, "groups_per_step": 512,
       "steps": steps, "save_steps": saves, "app": APP, "full_param": True, "extra_flags": extra_flags, "spawned": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
...
print(f"{what}: train {t.object_id}" + (f" eval {rec['eval']}" if "eval" in rec else "") + f" | run {run} | save {save} | lr {lr}")
