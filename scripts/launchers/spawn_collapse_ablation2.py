...
ARMS = {  # round 2: mechanism-level fixes (argparse: the LAST occurrence of a flag wins, so these override BASE's --adv-mode batch / --loss cispo / --kl-coef 0 / --entropy-target 0 / --groups-per-step 256)
    "rawadv":        "--lr 1e-5 --cispo-eps-max 5 --adv-mode none",
    "rawadv_gclip03": "--lr 1e-5 --cispo-eps-max 5 --adv-mode none --max-grad-norm 0.3",
    "groups512":     "--lr 1e-5 --cispo-eps-max 5 --groups-per-step 512",
    "ppoclip":       "--lr 1e-5 --cispo-eps-max 5 --loss ppo --clip-eps 0.2",
    "kl002":         "--lr 1e-5 --cispo-eps-max 5 --kl-coef 0.02",
    "enttarget":     "--lr 1e-5 --cispo-eps-max 5 --entropy-target 1.2 --entropy-coef 0.001",
    "rawadv_fresh":  "--lr 1e-5 --cispo-eps-max 5 --adv-mode none --fresh-optim",   # raw advantages WITH fresh Adam moments (the loaded state is calibrated to ~25x larger grads)
    "control_fresh": "--lr 1e-5 --cispo-eps-max 5 --fresh-optim",                   # fresh Adam moments alone, to separate that from the advantage change
}
...
ids = json.load(open("/home/celeste/shared/overnight/collapse_ablation2_ids.json")) if __import__("os").path.exists("/home/celeste/shared/overnight/collapse_ablation2_ids.json") else {}
for arm in only:
    name, save = f"rl_ablate2_{arm}", f"/data/ckpts_ablate2_{arm}"
...
json.dump(ids, open("/home/celeste/shared/overnight/collapse_ablation2_ids.json", "w"), indent=1)
