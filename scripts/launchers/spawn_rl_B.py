"""RL run B: init = old 500k realact+probes SFT (last5_rp/final); bank = random-ctx realact + cluster probes. 8xB200 (2 rollout + 6 trainer), 16x256."""
import modal
...
extra = (f"--recipe scalerl --loss cispo --cispo-eps-max 5 --loss-agg prompt --adv-mode batch --zero-var-filter --npr-threshold 0.9 --npr-pass-cos 0.7 --max-lag 8 --fp32-head --autocast-bf16 --length-control penalty "
         ...
         f"--run-name rl_B_randctx_probes_from_rp500k --save-dir /data/ckpts_rl_B_randctx_probes")
c = modal.Function.from_name("maemm-rl-disagg", "train").spawn(n_rollout=2, n_trainer=6, total_steps=400, extra_args=extra, pool_dir="/data/banks/rl_randctx_probes"); print(c.object_id)
