...
Idempotent: every stage is recorded in ids.json; rerun = resume. Discord on launches / completion / failure.
    ...
    cd /tmp && nohup python3 ~/maemm-pub-simple2m/scripts/launchers/simple2m_20m_chain_driver.py >> ~/shared/overnight/simple2m_20m/driver.out 2>&1 &
"""
import json
import math
import os
...
IDS = os.path.expanduser("~/shared/overnight/simple2m_20m/ids.json")
...
APPS = {"bank": "maemm-sae2m-bank-s2m", "collect": "maemm-collect-bank-s2m", "store": "maemm-acts-ufw-s2m", "long": "maemm-ufw-long-bank-s2m",
        "compose": "maemm-mix-5m-bank-s2m", "sft": "maemm-sft-fullft-s2m", "eval": "maemm-eval-ckpt-s2m-c", "rl": "maemm-rl-disagg-fullparam-s2m-c"}   # -c = centered scorer/reward (fix f9b5095)
...
# --length-bucket (NEW vs the 8M chain, user: "full ft with data shuffle"): every optimizer step is an i.i.d. window of 4,096 examples
# from the (already globally shuffled) mix, length-sorted only INSIDE the step into its 16 micro-batches. The 8M chain used the legacy
# default: all rows length-sorted into homogeneous micro-batches, micro-batches shuffled, 16 consecutive ones per step -> each step
# held only 16 distinct target lengths (SAE max-act windows and full-context activation targets have different length profiles).
...
def discord(m):
    subprocess.run(["notify-discord", f"simple2m-20m: {m}"])
...
if not stage(d, "compose_sft"):
    for need in ("/banks/sae2m_sft/build_stats.json", "/banks/ufw_ctx8_64_sft_4m_v3/build_stats.json"):
        assert vol_has(need), f"reused simple2m bank missing: {need}"
    wait_collect(d, "coll_sft_ctx8_64_12m", "ufw_ctx8_64_sft_12m", 12_000_000, 62)
    st = vol_get_json("/banks/sae2m_sft/build_stats.json"); fam = st["families"]
    n_enc, n_dec = int(fam["sae2m"]), int(fam["sae2m_dec"])
    acts_avail = int(vol_get_json("/banks/ufw_ctx8_64_sft_4m_v3/build_stats.json")["n_examples"]) + int(vol_get_json("/banks/ufw_ctx8_64_sft_12m/build_stats.json")["n_examples"])
    n_sae = 4_000_000                                                    # 2M encoder + 2M decoder rows = 20% of the corpus
    n_acts = min(4 * n_sae, acts_avail)                                  # 80%: short-ctx activations (16M target)
    spec = [{"name": "acts_ufw_sft", "banks": ["/data/banks/ufw_ctx8_64_sft_4m_v3", "/data/banks/ufw_ctx8_64_sft_12m"], "dtype": "f16", "families": ["realact"], "n": n_acts,
             "all_one_family": "realact", "fresh": "Ultra-FineWeb docs [5.5M, 5.625M) + [6.1M, ~6.48M) ordered stream; standalone 64-token windows, ctx 8-64, target = FULL context"},
    ...
    if "compose_sft" not in d:
        c = modal.Function.from_name(APPS["compose"], "build").spawn(out_name=SFT_MIX, spec_json=json.dumps(spec), seed=2050, threads=48,
    ...
    discord(f"SFT mix composed (80/20 acts/SAE): {st['n_examples']} rows {st['families']} -> launching full-FT SFT from base")
