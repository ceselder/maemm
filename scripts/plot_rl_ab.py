...
<h2>Does the pretrain matter? Same RL recipe from two starting points</h2>
<figure><img src="rl_pretrain_effect.png" alt="pretrain effect on RL"><figcaption>Same ScaleRL recipe (lr 7e-6, last-5 cosine reward) on all-families banks, started either from the
500k realact+probes SFT (two earlier runs, 2048 rollouts/step) or from the 23M-activation pretrain + 1.1M all-families midtrain (RL-C, 4096 rollouts/step); x = rollouts seen.
Data: data/rl_pretrain_effect.json; script scripts/plot_rl_pretrain_effect.py.</figcaption></figure>
