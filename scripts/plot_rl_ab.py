...
RUNS = {
    ...
    "C: init 23M-realact SFT + 1.1M all-families SFT, bank all-families mix": {"train": "oo7dvzz7", "eval": "rl_C_mix1m_from_realact23m_mixsft_eval", "color": "#2a7f62"},
}
...
fig.suptitle("Held-out fidelity per RL checkpoint: A (realact init) leads on real activations, B keeps SAE ~0.75, and C (realact SFT + all-families SFT, all-families bank) "
             "recovers SAE to 0.79 by step 100 despite a worse-scoring init", fontsize=10, y=0.995)
...
<div class="tldr"><b>TL;DR</b> A best mean_all {bA['eval/mean_all']:.3f} @ {bA['ckpt_step']}, B best {bB['eval/mean_all']:.3f} @ {bB['ckpt_step']}, C (running) best {bC['eval/mean_all']:.3f} @ {bC['ckpt_step']} with SAE {bC['eval/sae/norm_act']:.3f}.
...
(23M examples; RL-A, whose bank is real activations only), SFT on the 500k realact+probes datamix (two earlier runs on the all-families bank, 2048 rollouts/step), and SFT on real activations (23M)
continued on the 1.1M all-families datamix (RL-C, all-families bank, 4096 rollouts/step). All 36 numeric eval/locality metrics logged per checkpoint are shown, including the SAE
unverbalized fraction, fired fraction, mean rank and MRR. Dotted lines: each init before RL. Data: data/rl_pretrain_effect.json; script scripts/plot_rl_pretrain_effect.py.</figcaption></figure>
